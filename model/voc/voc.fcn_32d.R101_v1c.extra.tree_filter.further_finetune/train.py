...
with Engine(custom_parser=parser) as engine:
    ...
    init_weight(model.business_layers, nn.init.kaiming_normal_,
                BatchNorm2d, config.bn_eps, config.bn_momentum,
                mode='fan_in', nonlinearity='relu')
    # init_weight(model.embed_layers, nn.init.normal_,
    #             BatchNorm2d, config.bn_eps, config.bn_momentum,
    #             mean=0.0, std=0.01)

    # state_dict = torch.load(config.pretrained_model)
    state_dict = torch.load(config.pretrained_model,
                            map_location=lambda storage, loc: storage.cuda(engine.local_rank))
    ...
    params_list = group_weight(params_list, model.backbone,
                               BatchNorm2d, base_lr)
    for module in model.business_layers:
        params_list = group_weight(params_list, module, BatchNorm2d,
                                   base_lr * config.business_lr_ratio)
    ...
    for epoch in range(engine.state.epoch, config.nepochs):
        ...
        for idx in pbar:
            ...
            for i in range(2, len(optimizer.param_groups)):
                optimizer.param_groups[i]['lr'] = lr * config.business_lr_ratio
